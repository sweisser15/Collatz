...
def collatz_eval(i, j):
    ...
    # If i and j land exactly on a 1000 digit interval
    if (j - i) % 1000 == 0 and (j % 1000 == 0):
        maxVal = 0
        for a in range(indi - 1, indj - 1):
            if eCache[a] > maxVal:
                maxVal = eCache[a]
        return maxVal

    # If i and j are more than 1000 digits away
    if indj - indi >= 1:
        maxVal = 0
        # If i lands exactly at the start of a 1000 digit interval
        if i % 1000 == 0:
            for x in range(indi - 1, indj - 1):
                if eCache[x] > maxVal:
                    maxVal = eCache[x]
            upper = collatz_eval_helper(indj * 1000, j)
            if upper > maxVal:
                maxVal = upper
        # If i lands before the start of a 1000 digit interval
        else:
            for x in range(indi, indj - 1):
                if eCache[x] > maxVal:
                    maxVal = eCache[x]
            upper = collatz_eval_helper(indj * 1000, j)
            if upper > maxVal:
                maxVal = upper
            lower = collatz_eval_helper(i, (indi+1) * 1000)
            if lower > maxVal:
                maxVal = lower
        return maxVal

    # If i and j are less than 1000 digits away
    else:
        maxVal = collatz_eval_helper(i, j)
        return maxVal


def collatz_eval_helper(i, j):
    """
    helper function for collatz_eval that returns
    the max cycle length of the range [i, j] within 100 digits
    """
    max_val = 0
